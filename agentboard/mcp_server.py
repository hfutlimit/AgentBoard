...
# Phase 6: helper imports (split from mcp_server.py to features/mcp/)
from .features.mcp import admin as mcp_admin
from .features.mcp import auth as mcp_auth
from .features.mcp import documents as mcp_documents
from .features.mcp import notifications as mcp_notifications
from .features.mcp import projects as mcp_projects
...
# the helper to be reachable on the mcp_server module itself. We re-export
# every helper from the feature modules so legacy callers keep working.
# Generated by re-reading the feature modules' globals.
import sys as _sys
_module_self = _sys.modules[__name__]
for _mod in (mcp_admin, mcp_auth, mcp_documents, mcp_notifications,
             mcp_projects, mcp_proposals, mcp_scheduling, mcp_work_items):
    for _name in dir(_mod):
        if _name.startswith("_") and not _name.startswith("__"):
            _obj = getattr(_mod, _name)
            if callable(_obj) and not hasattr(_module_self, _name):
                setattr(_module_self, _name, _obj)
...
def list_projects(limit: int | None = None, offset: int = 0) -> list:
    ...
    return mcp_projects._proj_list(limit=limit, offset=offset)


@mcp.tool()
def get_project(project_id: int) -> dict:
    """获取 Project 详情。"""
    return mcp_projects._proj_get(project_id)


@mcp.tool()
def create_project(name: str, key: str | None = None, description: str = "") -> dict:
    """创建项目。name 必填，key 为短码，description 为 markdown。"""
    return mcp_projects._proj_create(name, key, description)
...
def update_project(project_id: int, name: str | None = None, key: str | None = None,
    ...
    return mcp_projects._proj_update(project_id, fields)


@mcp.tool()
def delete_project(project_id: int) -> dict:
    """删除 Project 及其全部 Epic、Story、Task 和评论。"""
    return mcp_projects._proj_delete(project_id)


@mcp.tool()
def list_epics(project_id: int, limit: int | None = None, offset: int = 0) -> list:
    """分页列出指定 Project 下的 Epic。"""
    return mcp_projects._epic_list(project_id, limit=limit, offset=offset)


@mcp.tool()
def create_epic(project_id: int, title: str, description: str = "") -> dict:
    """在指定项目下创建 Epic。"""
    return mcp_projects._epic_create(project_id, title, description)
...
def create_story(epic_id: int, title: str, description: str = "",
    ...
    """
    return mcp_projects._story_create(epic_id, title, description, needs_design)


@mcp.tool()
def list_stories(epic_id: int, limit: int | None = None, offset: int = 0) -> list:
    """分页列出指定 Epic 下的 Story。"""
    return mcp_projects._story_list(epic_id, limit=limit, offset=offset)


@mcp.tool()
def get_epic(epic_id: int) -> dict:
    """获取 Epic 详情。"""
    return mcp_projects._epic_get(epic_id)
...
def update_epic(epic_id: int, title: str | None = None, description: str | None = None,
    ...
    return mcp_projects._epic_update(epic_id, fields)


@mcp.tool()
def delete_epic(epic_id: int) -> dict:
    """删除 Epic（级联删除其 Stories / Tasks）。"""
    return mcp_projects._epic_delete(epic_id)


@mcp.tool()
def get_story(story_id: int) -> dict:
    """获取 Story 详情。"""
    return mcp_projects._story_get(story_id)
...
def update_story(story_id: int, title: str | None = None, description: str | None = None,
    ...
    return mcp_projects._story_update(story_id, fields)
...
def delete_story(story_id: int) -> dict:
    ...
    return mcp_projects._story_delete(story_id)


@mcp.tool()
def list_tasks(story_id: int, limit: int | None = None, offset: int = 0) -> list:
    """分页列出指定 Story 下的 Task/Bug。"""
    return mcp_work_items._task_list(story_id, limit=limit, offset=offset)
...
def get_task(task_id: int) -> dict:
    ...
    return mcp_work_items._task_get(task_id)
...
def update_task(task_id: int, title: str | None = None, description: str | None = None,
    ...
    return mcp_work_items._task_update(task_id, fields)


@mcp.tool()
def delete_task(task_id: int) -> dict:
    """删除 Task/Bug 及其评论。"""
    return mcp_work_items._task_delete(task_id)


@mcp.tool()
def set_task_spec(task_id: int, spec: str) -> dict:
    """设置任务 spec（OpenSpec/Superpowers 风格 markdown）。"""
    return mcp_work_items._task_update(task_id, {"spec": spec})


@mcp.tool()
def get_task_spec(task_id: int) -> dict:
    """读取任务 spec 原文。"""
    t = mcp_work_items._task_get(task_id)
    if "error" in t:
        return t
...
def append_task_spec(task_id: int, text: str) -> dict:
    ...
    return mcp_work_items._task_append_spec(task_id, text)
...
def search_tasks(project_id: int | None = None, epic_id: int | None = None,
    ...
    return mcp_work_items._task_search(dict(project_id=project_id, epic_id=epic_id, story_id=story_id,
...
def list_comments(task_id: int) -> list | dict:
    ...
    return mcp_work_items._comment_list(task_id)
...
def spec_proposal(task_id: int, title: str, background: str, goal: str,
    ...
    return mcp_work_items._task_update(task_id, {"spec": md})
...
def generate_tasks_from_spec(task_id: int) -> list:
    ...
    """
    return mcp_work_items._task_generated(task_id)
...
def list_sprints(project_id: int, limit: int | None = None, offset: int = 0) -> list:
    ...
    return mcp_projects._sprint_list(project_id, limit=limit, offset=offset)


@mcp.tool()
def get_sprint(sprint_id: int) -> dict:
    """获取 Sprint 详情（含 goal、日期、状态）。"""
    return mcp_projects._sprint_get(sprint_id)
...
def create_sprint(project_id: int, title: str, goal: str = "",
    ...
    return mcp_projects._sprint_create(project_id, title, goal=goal, start_date=start_date, end_date=end_date)
...
def list_schedules(project_id: int, limit: int | None = None, offset: int = 0) -> list:
    ...
    return mcp_scheduling._schedule_list(project_id, limit=limit, offset=offset)


@mcp.tool()
def get_schedule(schedule_id: int) -> dict:
    """获取 AgentSchedule 详情。"""
    return mcp_scheduling._schedule_get(schedule_id)
...
def create_schedule(project_id: int, title: str, schedule_type: str = "cron",
    ...
    return mcp_scheduling._schedule_create(project_id, title, schedule_type=schedule_type,
...
def update_schedule(schedule_id: int, title: str | None = None,
    ...
    return mcp_scheduling._schedule_update(schedule_id, fields)


@mcp.tool()
def delete_schedule(schedule_id: int) -> dict:
    """删除 AgentSchedule（级联删除运行记录）。"""
    return mcp_scheduling._schedule_delete(schedule_id)
...
def list_runs(schedule_id: int, limit: int | None = None, offset: int = 0) -> list:
    ...
    return mcp_scheduling._run_list(schedule_id, limit=limit, offset=offset)


@mcp.tool()
def get_run(run_id: int) -> dict:
    """获取 AgentRun 详情（含 output/error_message）。"""
    return mcp_scheduling._run_get(run_id)
...
def create_run(schedule_id: int, task_id: int | None = None,
    ...
    return mcp_scheduling._run_create(schedule_id, task_id=task_id, idempotency_key=idempotency_key)
...
def update_run(run_id: int, status: str | None = None, output: str | None = None,
    ...
    return mcp_scheduling._run_update(run_id, fields)


@mcp.tool()
def delete_run(run_id: int) -> dict:
    """删除运行记录。"""
    return mcp_scheduling._run_delete(run_id)
...
def auth_register(username: str, password: str) -> dict:
    ...
    return mcp_auth._auth_register(username, password)


@mcp.tool()
def auth_login(username: str, password: str) -> dict:
    """登录 AgentBoard 并返回带有效期的 Token。"""
    return mcp_auth._auth_login(username, password)
...
def list_attachments(task_id: int) -> list | dict:
    """列出任务的所有附件元数据（不含文件内容）。"""
    return mcp_work_items._attachment_list(task_id)


@mcp.tool()
def get_attachment_info(attachment_id: int) -> dict:
    """获取附件元数据（id、文件名、MIME、大小、上传时间）。"""
    return mcp_work_items._attachment_get(attachment_id)
...
def get_project_stats(project_id: int) -> dict:
    ...
    return mcp_projects._project_stats(project_id)
...
def claim_task(task_id: int, agent_name: str = "agent") -> dict:
    ...
    """
    return mcp_scheduling._agent_claim_task(task_id, agent_name)
...
def heartbeat(run_id: int, status: str = "running") -> dict:
    ...
    """
    return mcp_scheduling._agent_heartbeat(run_id, status)
...
def complete_run(run_id: int, output: str, status: str = "success",
    ...
    """
    return mcp_scheduling._agent_complete_run(run_id, output, status, error_message)
...
def sync_status(task_id: int, status: str, comment: str | None = None,
    ...
    if comment:
        ...
        mcp_work_items._comment_create(task_id, author=author, content=comment)
    return result
...
def list_members(project_id: int, limit: int = 50, offset: int = 0) -> list:
    ...
    resp = mcp_projects._member_list(project_id, limit=limit, offset=offset)
...
def add_member(project_id: int, user_id: int | None = None, username: str | None = None, role: str = "member") -> dict:
    ...
    - role: member / admin
    """
    return mcp_projects._member_add(project_id, user_id=user_id, username=username, role=role)


@mcp.tool()
def remove_member(project_id: int, user_id: int) -> dict:
    """移除项目成员（需 owner 或管理员权限，owner 不能移除自己）。"""
    return mcp_projects._member_remove(project_id, user_id)


@mcp.tool()
def update_member_role(project_id: int, user_id: int, role: str) -> dict:
    """更新成员角色（需 owner 或管理员权限）。role: member / admin"""
    return mcp_projects._member_update_role(project_id, user_id, role)
...
def list_notifications(limit: int = 20, offset: int = 0, unread_only: bool = False) -> dict:
    ...
    resp = mcp_notifications._notification_list(limit=limit, offset=offset, unread_only=unread_only)
    return resp if isinstance(resp, dict) else {"items": resp}
...
def notification_unread_count() -> dict:
    """返回当前用户未读通知数量。"""
    return mcp_notifications._notification_unread_count()


@mcp.tool()
def mark_notification_read(notification_id: int) -> dict:
    """标记单条通知为已读。"""
    return mcp_notifications._notification_mark_read(notification_id)


@mcp.tool()
def mark_all_notifications_read() -> dict:
    """标记当前用户全部通知为已读。"""
    return mcp_notifications._notification_mark_all_read()


@mcp.tool()
def delete_notification(notification_id: int) -> dict:
    """删除单条通知。"""
    return mcp_notifications._notification_delete(notification_id)
...
def admin_list_users(limit: int = 50, offset: int = 0) -> dict:
    """（管理员）列出全部用户。"""
    resp = mcp_admin._admin_list_users(limit=limit, offset=offset)
    return resp if isinstance(resp, dict) else {"items": resp}
...
def admin_set_user_admin(user_id: int, is_admin: bool) -> dict:
    ...
    return mcp_admin._admin_update_user(user_id, is_admin)


@mcp.tool()
def admin_list_projects(limit: int = 50, offset: int = 0) -> dict:
    """（管理员）列出全部项目。"""
    resp = mcp_admin._admin_list_projects(limit=limit, offset=offset)
    return resp if isinstance(resp, dict) else {"items": resp}
...
def admin_delete_project(project_id: int) -> dict:
    ...
    return mcp_admin._admin_delete_project(project_id)
...
def create_document(project_id: int, title: str, content: str = "",
    ...
    return mcp_documents._doc_create(project_id, title, content=content, type=type, status=status,
...
def list_documents(project_id: int | None = None, type: str | None = None,
    ...
    return mcp_documents._doc_list(project_id=project_id, type=type, status=status, q=q,
...
def update_document(document_id: int, title: str | None = None,
    ...
    return mcp_documents._doc_update(document_id, fields)


@mcp.tool()
def delete_document(document_id: int) -> dict:
    """删除文档（级联删除其评论）。"""
    return mcp_documents._doc_delete(document_id)
...
def create_document_folder(project_id: int, name: str,
    ...
    return mcp_documents._folder_create(project_id, name, parent_id=parent_id)


@mcp.tool()
def list_document_folders(project_id: int | None = None) -> list:
    """列出文档文件夹树（可按 project_id 过滤）。"""
    return mcp_documents._folder_list(project_id=project_id)
...
def add_document_comment(document_id: int, author: str, content: str,
    ...
    return mcp_documents._doc_comment_create(document_id, author, content, author_id=author_id)


@mcp.tool()
def list_document_comments(document_id: int) -> list | dict:
    """按时间顺序读取文档评论，供多成员 / 多 Agent 互相 review。"""
    return mcp_documents._doc_comment_list(document_id)


@mcp.tool()
def update_document_comment(comment_id: int, content: str, author: str) -> dict:
    """编辑文档评论：仅作者（成员或 Agent 账号）可编辑自己的评论。"""
    return mcp_documents._doc_comment_update(comment_id, content, author)


@mcp.tool()
def delete_document_comment(comment_id: int) -> dict:
    """删除指定文档评论。"""
    return mcp_documents._doc_comment_delete(comment_id)
...
def search_documents(project_id: int | None = None, q: str | None = None,
    ...
    return mcp_documents._doc_list(project_id=project_id, q=q, type=type, status=status,
                     limit=limit, offset=offset)
...
def get_project_memory(project_id: int, agent: str | None = None) -> dict:
    ...
    供 Agent 会话启动时一次自动加载。
    """
    docs = mcp_documents._doc_list(project_id=project_id, type="memory", limit=100)
    ...
    if agent:
        docs = [d for d in docs
                ...
                or d.get("title") == mcp_documents._memory_title(agent)]
    for d in docs:
...
def append_agent_memory(project_id: int, content: str, agent: str | None = None) -> dict:
    ...
    """
    title = mcp_documents._memory_title(agent)
    docs = mcp_documents._doc_list(project_id=project_id, type="memory", limit=100)
    target = None
    ...
    if target is not None:
        ...
        merged = (old + "\n\n" + content).strip()
        mcp_documents._doc_update(target["id"], {"content": merged})
        return {"document_id": target["id"], "title": title, "appended": True,
                "content_length": len(merged)}
    created = mcp_documents._doc_create(project_id, title, content=content, type="memory")
...
def proposal_get(proposal_id: int) -> dict:
    ...
    if _is_http_error(rounds):
        return rounds
    return mcp_proposals._proposal_replay(p, rounds)
...
def proposal_finalize(proposal_id: int, converged_spec: str) -> dict:
    ...
    if _is_http_error(r):
        return r
    return mcp_proposals._proposal_status(proposal_id, "converged")


@mcp.tool()
def proposal_fail(proposal_id: int, error: str) -> dict:
    """标记提案分析失败并记录原因，供后续回退 queued 重投。"""
    return mcp_proposals._proposal_status(proposal_id, "failed", error=error or "unspecified failure")
