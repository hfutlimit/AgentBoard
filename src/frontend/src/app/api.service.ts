...
import { ApiErrorBody, ApiKeyInfo, Attachment, AuthResult, Comment, Epic, KanbanBoard, Notification, OverviewStats, PagedResult, Project, ProjectMember, ProjectStats, ReviewStats, ReviewTimeoutResult, Sprint, Story, Task, AgentSchedule, AgentRun, TaskDependencies, AuditLog, UserProfile, WebhookConfig, DocumentItem, DocumentCommentItem, DocumentFolder, DocumentRevisionItem, DocumentType, DocumentStatus, ProposalItem, ProposalRoundItem, ProposalQuestionItem, ProposalStatus, TicketRequestItem, TicketType, TicketItem, AgentRow, StoryStatusHistoryRow, AgentBehaviorConfigPayload, EffectiveBehaviorConfig, BehaviorPreviewResponse, ProjectLearningItem } from './models';
...
export class ApiService {
  ...
  async streamRunEvents(
    ...
    const waitForReconnect = async (delay: number): Promise<void> => {
    ...
    while (!signal?.aborted) {
      ...
      try {
        ...
        if (signal?.aborted) return;
        if (error instanceof Error && error.message.startsWith('Run event stream failed: HTTP')) {
          throw error;
        }
      }
  ...
  createProposal(body: { project_id: number; title: string; content?: string; auto_create_ticket?: boolean; target_epic_id?: number | null }) {
    return this.request<ProposalItem>('POST', '/api/proposals', body).pipe(
      tap(() => apiCache.invalidatePrefix('/api/proposals'))
    );
  }
  updateProposal(id: number, body: { title?: string; content?: string; converged_spec?: string; auto_create_ticket?: boolean; target_epic_id?: number | null }) {
  ...
  updateProjectBehavior(projectId: number, payload: AgentBehaviorConfigPayload, workType?: string) {
    const params = workType ? { work_type: workType } : undefined;
    return this.request<{ status: string; id: number }>(
      'PUT',
  ...
  resetProjectBehavior(projectId: number, workType?: string) {
    const params = workType ? { work_type: workType } : undefined;
  ...
  getAgentBehavior(projectId: number, agentId: number, workType?: string) {
    const params = workType ? { work_type: workType } : undefined;
    return this.request<EffectiveBehaviorConfig>(
      'GET',
  ...
  resetAgentBehavior(projectId: number, agentId: number, workType?: string) {
    const params = workType ? { work_type: workType } : undefined;
    return this.request<{ status: string; deleted: boolean }>(
      'DELETE',
      `/api/projects/${projectId}/agents/${agentId}/behavior`,
  ...
  getProjectLearnings(projectId: number, category?: string, workType?: string) {
    const params: Record<string, string> = {};
