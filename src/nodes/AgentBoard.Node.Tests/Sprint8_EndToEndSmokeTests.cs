...
///
/// Pair with <c>scripts/install-node.ps1 -SmokeRun</c> (TODO) for an
...
public sealed class Sprint8_EndToEndSmokeTests
{
    ...
    public async Task Startup_recovery_reenqueues_pending_inbox_rows()
    {
        ...
        // DB on its own.
        var newChannel = new ExecutionChannel(Options.Create(new NodeOptions
        {
            Id = "test-worker",
    ...
    public void WorkerIdentity_centralizes_fallback_for_empty_config()
    {
        ...
        var opts = Options.Create(new NodeOptions { Id = "", Version = "1.0.0" });
        ...
        var configured = new WorkerIdentity(Options.Create(new NodeOptions { Id = "prod-01", Version = "1.0.0" }));
    ...
    public async Task Dispatcher_refills_pending_after_drain_when_above_capacity()
    {
        ...
        var smallChannel = new ExecutionChannel(Options.Create(new NodeOptions
        {
            Id = "test-worker",
