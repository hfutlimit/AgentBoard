...
public sealed class Sprint6_WorkerStateTests : IDisposable
{
    ...
    {
        var state = new WorkerState(Options.Create(new NodeOptions
        {
            Id = "prod-pc-01",
            Version = "1.0.0",
        }), new WorkerIdentity(Options.Create(new NodeOptions
        {
            Id = "prod-pc-01",
            Version = "1.0.0",
        })));
    ...
    {
        // Empty Id is the NodeOptions default (Environment.MachineName is
    ...
    private WorkerState NewState() =>
        new(Options.Create(new NodeOptions { Id = "test-worker", Version = "1.0.0" }),
            new WorkerIdentity(Options.Create(new NodeOptions { Id = "test-worker", Version = "1.0.0" })));
