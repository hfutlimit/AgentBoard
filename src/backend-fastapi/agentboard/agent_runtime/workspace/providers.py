...
------------------------------------------
1. ``agent_runtime/invokers.py::_resolve_project_cwd`` 是当前的"项目本地
   目录"真源（读 ``AGENTBOARD_LOCAL_MAPPINGS`` / ``tmp/project-mappings.json``）。
   ``SubprocessAgentInvoker.invoke`` 里拿到 cwd 之后，改为调用
   :func:`prepare_workspace`：``handle.path`` 取代裸 cwd 传给 subprocess，
   ``handle.git_args`` 在 agent 启动前执行一次。
2. ``agent_runtime/handlers/*.py`` 的 ``_resolve_project_dir``（clarify /
