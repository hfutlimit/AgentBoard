...
- ``CallableProcessorInvoker``：测试与内嵌策略用（任意可调用 → 决策）；
- ``SubprocessProcessorInvoker``：真实 CLI（WorkBuddy / Claude Code / Codex）子进程；
- ``extract_decision_json`` / ``split_command``：子进程输出解析与命令行拆分。

Story 243（Epic 122 S5）：``SubprocessProcessorInvoker`` 支持按 ``project_id``
解析本机工作目录（``AGENTBOARD_LOCAL_MAPPINGS`` JSON，由本机配置台
``processor_portal.py`` 写入）——任务属于哪个项目，Agent 就在映射的本地目录里跑。
"""
...
log = logging.getLogger("agentboard.processors.invokers")
...
class RoutedSubprocessInvoker:
    """按 context["action"] 路由到子 SubprocessProcessorInvoker 的复合 Invoker。

    单一 Worker 进程、单一认领身份；多通道只是「同一个 prompt 喂不同的子 CLI」。
    子进程并行上限 = 1（受 SubprocessProcessorInvoker 自身 prompt 串行喂入的语义控制），
    ...
    def __init__(self, commands: dict[str, str] | None = None,
        ...
        self._children: dict[str, SubprocessProcessorInvoker] = {
            alias: (fallback if fallback is not None and alias == self.fallback_alias
                    else SubprocessProcessorInvoker(cmd=cmds[alias], timeout=timeout,
                                               cwd=cwd, env=env,
...
# 由 ProposalProcessor 在构造时注入 prompt_builder，解耦提示词与调用器。
_prompt_builder: Callable[[dict], str] | None = None


def set_prompt_builder(fn: Callable[[dict], str]) -> None:
    """注入全局 prompt 构建函数（ProposalProcessor 构造时调用一次）。"""
...
class CallableProcessorInvoker:
...
def _resolve_project_cwd(context: dict, fallback: str | None) -> str | None:
    ...
    Story 243（Epic 122 S5）：映射由本机配置台（``processor_portal.py``
    ...
    if not raw:
        ...
        # 当前运行时代码位于 src/backend-fastapi/agentboard/processors，
...
class ComplianceEnforcingInvoker:
    ...
    def __init__(self, delegate: ProcessorInvoker):
...
class SubprocessProcessorInvoker:
