...
兼容性：``ProposalProcessor(config, invoker=..., client=...)`` 调用契约不变；
``agentboard.processors`` 模块符号（ProposalProcessor / ProcessorConfig / AgentDecision /
SubprocessProcessorInvoker / CallableProcessorInvoker / split_command / extract_decision_json）
...
from .config import AgentDecision, ProcessorConfig, WorkerError
...
log = logging.getLogger("agentboard.processors")

__all__ = [
    "ProposalProcessor", "ProcessorConfig", "AgentDecision",
    "SubprocessProcessorInvoker", "CallableProcessorInvoker",
...
def _outcome_from_result(result: Any, default: str = "handled") -> str:
    ...
    if name == "SUCCESS":
        # 统一执行内核不应破坏 ProposalProcessor 历史公开 outcome：
...
class ProposalProcessor:
    ...
    def __init__(self, config: ProcessorConfig, invoker: ProcessorInvoker | None = None,
                 client: httpx.Client | None = None):
        from .config import ProcessorInvoker  # 协议，延迟导入避免循环
        # P1 架构收口（2026-08-26 review）：ProcessorCoordinator 是统一执行内核，
        # ProposalProcessor 薄化为 main loop / broker adapter / backward-compat facade。
        # config.use_coordinator=False 时所有 handle* 方法走旧 _handlers 路径（灰度用），
        # 默认 True → 所有 ExecutionCommand 走 coordinator.dispatch()。
        from .coordinator import ProcessorCoordinator
        self._coordinator = ProcessorCoordinator(
    ...
    @staticmethod
    def _default_invoker(config: ProcessorConfig) -> "ProcessorInvoker":
        # 优先级（2026-08-25 多通道路由）：
        #   1. AGENTBOARD_WORKER_AGENT_COMMANDS（多 alias 路由表）→ RoutedSubprocessInvoker
        #   2. AGENTBOARD_WORKER_AGENT_CMD（旧单字符串）→ SubprocessProcessorInvoker
        # 都没配 → fail-fast
        from .invokers import (
        ...
        return SubprocessProcessorInvoker(config.agent_cmd, timeout=config.agent_timeout)
    ...
    def __enter__(self) -> "ProposalProcessor":
        return self
