...
接入点建议
----------
``processors/invokers.py::SubprocessProcessorInvoker`` 目前直接 ``subprocess.run``
...
两个 provider 用 Protocol 描述（与 ``processors/config.py`` 的
``ProcessorInvoker`` 同一套约定），便于后续替换实现而不动调用方。
"""
