...
放置位置说明
------------
本包放在 ``processors/workspace`` 而不是 ``features/workspace``：workspace
是**执行期基础设施**（无 DB 实体、无 HTTP 路由），与同级的
``processors/behavior`` 同性质；``features/*`` 是"实体 + router + service"
...
（``processors/invokers.py`` 的 subprocess 路径）负责。这样本包可单测、
可逐步接入，不会引入"改了一半的副作用"。
...
class RuntimeProviderKind(StrEnum):
    ...
    ``host``   直接在 Worker 宿主机跑（默认，与既有 SubprocessProcessorInvoker 一致）；
