...
独立模块承载：异常体系、``ProcessorConfig``、``AgentDecision``、Agent 决策
...
class ProcessorConfig:
    ...
    # - True  → ProposalProcessor 把 ExecutionCommand 转给 ProcessorCoordinator.dispatch()
...
class AgentDecision:
    ...
    # 包含一个源码/配置/测试文件。CallableProcessorInvoker 仍作为纯测试/嵌入 seam。
...
class ProcessorInvoker(Protocol):
