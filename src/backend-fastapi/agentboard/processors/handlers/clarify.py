...
log = logging.getLogger("agentboard.processors.clarify")
...
class ClarifyHandler(BaseWorkHandler):
    ...
    # ---------- HTTP 辅助（镜像 ProposalProcessor 的 _request/_get_json） ----------
    ...
    def _resolve_project_dir(self, project_id: Any) -> str:
        """从与 SubprocessProcessorInvoker 同一份本地映射文件查 project_dir。"""
        if not project_id:
            return ""
        try:
    ...
    def handle(self, proposal: dict, invoker: ProcessorInvoker) -> str:
