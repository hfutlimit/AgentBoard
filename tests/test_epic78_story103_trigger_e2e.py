"""
Epic 78 Story 103 — Trigger E2E 冒烟（Playwright + REST）

验证 Story 103（WebhookTrigger / trigger_run）交付未破坏 Web 服务：
1. executor 模块可导入，ADAPTERS 含 workbuddy/qoder → WebhookTrigger；
2. 真实拉起 API + Web（临时 SQLite），Chromium 登录后项目/看板核心渲染 0 报错。

（trigger_run 全链路 pending→running→webhook→外部回写→finalize 已由
tests/test_epic78_story103_trigger.py 单测覆盖；本 E2E 不跨进程操作同一
SQLite 文件，避免 Windows 文件锁竞争导致的不可控卡顿。）
...
def _seed_user_and_project(api_base: str) -> dict:
    ...
    reg = post("/api/auth/register", {"username": "u103", "password": "p103pass"})
    token = reg["token"]
    proj = post("/api/projects", {"name": "P103E2E", "key": "P103E"}, token=token)
    epic = post(f"/api/projects/{proj['id']}/epics",
                {"title": "Epic103", "description": ""}, token=token)
    story = post(f"/api/epics/{epic['id']}/stories",
                 {"title": "Story103", "description": ""}, token=token)
    task = post(f"/api/stories/{story['id']}/tasks",
                {"title": "T103", "project_id": proj["id"],
                 "description": "", "spec": "## 需求\nx\n## 验收\n- ok",
                 "priority": "high"}, token=token)
    sch = post(f"/api/projects/{proj['id']}/schedules",
               {"title": "s103", "schedule_type": "once"}, token=token)
    run = post(f"/api/schedules/{sch['id']}/runs",
               {"task_id": task["id"], "idempotency_key": "e2e103"}, token=token)
    return {"token": token, "project_id": proj["id"], "run_id": run["id"]}


def test_executor_module_importable():
    """executor 模块可导入，ADAPTERS 含 workbuddy/qoder → WebhookTrigger。"""
    from agentboard.executor import (
        ADAPTERS, TRIGGER_AGENTS, WebhookTrigger, trigger_run,
    )

    assert "workbuddy" in ADAPTERS
    assert "qoder" in ADAPTERS
    assert ADAPTERS["workbuddy"] is WebhookTrigger
    assert ADAPTERS["qoder"] is WebhookTrigger
    assert "workbuddy" in TRIGGER_AGENTS
    assert callable(trigger_run)
...
def test_web_renders_no_errors(api_base, web_base):
    ...
    with sync_playwright() as pw:
        ...
        page.wait_for_function(
            "() => document.body.innerText.includes('P103E2E')", timeout=15000)
        body = page.inner_text("body")
        assert "P103E2E" in body

        page.click("text=P103E2E")
        page.wait_for_timeout(2000)
        body2 = page.inner_text("body")
        assert "Epic103" in body2 or "看板" in body2 or "Story103" in body2
