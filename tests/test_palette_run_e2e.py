"""
Epic 135 (v6.20) 命令面板接入 AgentRun 搜索 —— 端到端验证
环境：本地 Docker 栈（web 28080 / API 18000），也可被 tmp harness 覆写 WEB/API 复用
策略（真实后端）：
- admin（不存在则注册）建项目 + 创建唯一 token 命名的 AgentSchedule 定时计划 + 挂一条
  AgentRun（summary 含唯一 token，状态 success）
- 断言：
  1) Ctrl+K 打开命令面板
  2) 输入唯一 token -> 出现「运行」(cat-run) 分类结果；标题含 token；
     点击 -> /project/{id}/schedules 项目定时计划 Tab 渲染且面板关闭
  3) 输入无匹配 token -> 「无匹配命令」空态
  4) 0 pageerror / console error / .js+.css 404
- 测试末清理种子（DELETE /api/projects/{id} 级联删除计划与运行记录），不污染数据
"""
import json
import random
...
ADMIN_USER = "admin"
ADMIN_PASS = "admin123"
SEED = "__E2E_V620_" + str(random.randint(100000, 999999))
PROJ_NAME = "E2E Run Search " + str(random.randint(100000, 999999))
...
def seed_run(admin_token):
    """建项目 + 创建 cron 定时计划 + 挂一条 summary 含唯一 token 的 success AgentRun。"""
    ...
    sid = sch["id"]
    st, run = api("POST", f"/api/schedules/{sid}/runs", token=admin_token, body={})
    assert st == 201, f"create run {st}: {run}"
    rid = run["id"]
    st, run = api("PATCH", f"/api/runs/{rid}", token=admin_token,
                  body={"status": "success", "summary": SEED + "执行摘要"})
    assert st == 200, f"patch run {st}: {run}"
    return pid, sid, rid
...
def main():
    ...
    created = []
    try:
        pid, schedule_id, run_id = seed_run(admin_token)
        created.append(pid)
        print(f"[seed] project={pid} schedule={schedule_id} run={run_id} token={SEED}")
        ...
        with sync_playwright() as p:
            ...
            # 2) Run 搜索：输入唯一 token -> .cat-run 结果
            page.fill("#paletteInput", SEED)
            page.wait_for_selector(".palette-item-cat.cat-run", timeout=10000)
            run_items = page.locator(".palette-item-cat.cat-run")
            assert run_items.count() >= 1, "未出现 Run 搜索结果"
            print(f"[OK] Run 搜索 {run_items.count()} 条")

            # 点击 Run 结果 -> /project/{pid}/schedules 项目定时计划 Tab 渲染且面板关闭
            run_loc = page.locator(".palette-item", has_text="运行").first
            run_title = run_loc.inner_text()
            assert SEED in run_title, f"Run 结果标题不含 token: {run_title}"
            run_loc.click()
            ...
            print("[OK] 点击 Run 结果进入项目定时计划 Tab 且渲染")
