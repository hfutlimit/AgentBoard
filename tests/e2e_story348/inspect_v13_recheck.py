"""
AGB v13 聚焦复核 (第 30 次执行, Story 348 全站巡检)
暖机复核 v6 初判 txt=0 的 8 页 (epics/tasks/bugs/documents/dashboard/settings/agents/notifications),
判定真假空白 (冷启 lazy chunk 时序 artifact vs 真 bug); 复验已知 Bug #1428/#1430/#1431。
鲁棒模式: domcontentloaded + 轮询稳定 (禁用 networkidle, 应用常驻 WS); 每环节独立 try/except; 末强制写 JSON。
"""
...
TOKEN = login_token()
res = {"warm_recheck": [], "known_bug_recheck": {}, "responsive": [], "create_dialog": {}}
...
with sync_playwright() as p:
    ...
    # 1) 暖机复核 v6 初判 0 字符的 8 页
    for path in ["/epics","/tasks","/bugs","/documents","/dashboard","/settings","/agents","/notifications"]:
        try:
    ...
    try:
        ...
        try: page.screenshot(path=os.path.join(SHOT,"create_dialog_v13.png"))
...
with open(os.path.join(OUT,"report_v13_recheck.json"),"w",encoding="utf-8") as f:
