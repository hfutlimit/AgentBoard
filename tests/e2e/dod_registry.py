...
REGISTRY: list[DodEntry] = [
    ...
    # ── Workflow Run Foundation · AI Team Live Operations Dashboard (2026-09-11) ──────────
    # 7 slice 全做；本 entry 覆盖 slice 1 (workflow_runs + workflow_run_events 表 + 状态机 + service helper)
    ...
    DodEntry(
        ...
        acceptance=[
            ...
            "workflow_type schema **不**加 CHECK 约束，v1 仅 `story`，future `schedule`/`proposal`/`ticket`/`deployment` 落地无需 migration",
            "Status 状态机：`completed` / `failed` / `cancelled` 全部 terminal；`failed → running` 永远拒绝（dashboard 监控可信）",
            "Retry terminal workflow = 新建 run + `reopened_from_run_id` 关联，老 run 历史永久不变",
