...
## API / 状态机
- 任务状态 `PUT /api/tasks/{tid}/status`。真实迁移表（service.py TRANSITIONS，BFS 求最短路径勿硬编码）：`BACKLOG→TODO`；`TODO→IN_PROGRESS/BACKLOG/DONE`；`IN_PROGRESS→IN_REVIEW/VERIFYING/TODO/DONE`；`IN_REVIEW→DONE/IN_PROGRESS`；`VERIFYING→DONE/IN_PROGRESS`；`DONE→IN_PROGRESS/TODO`。**无 in_review→verifying**。A-22 例外：TODO/IN_PROGRESS→DONE、DONE→TODO。
