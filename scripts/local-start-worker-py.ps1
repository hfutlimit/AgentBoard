...
# PID     ：logs/worker-py.pids（scripts/local-stop-worker-py.ps1 使用）
# =============================================================================
...
if (-not (Test-Path $EnvFile)) {
    throw "缺少配置文件 $EnvFile —— 按 scripts/local-start-worker-py.ps1 头部说明创建"
}
...
if (Test-Path $PidFile) {
    ...
    if ($alive.Count -gt 0) {
        throw "已有 worker 在运行（$($alive -join ', ')）。先执行 scripts\local-stop-worker-py.ps1 再启动。"
    }
}
...
Start-PyWorker 'proposal' @('-m', 'agentboard.agent_runtime', '--loop')
...
Start-PyWorker 'portal' @('-m', 'agentboard.worker_portal', '--host', '127.0.0.1', '--port', $PortalPort)

Set-Content -Path $PidFile -Value $pids -Encoding UTF8
Write-Host "`npid 文件：$PidFile"
Write-Host "停止：scripts\local-stop-worker-py.ps1"
