...
- **MCP 服务**（`agentboard/mcp_server.py`）：默认 httpx 调 API，可切换 `AGENTBOARD_MCP_BACKEND=db` 直连数据库；支持本地 stdio 与远程 Streamable HTTP。
...
> MCP 默认 `AGENTBOARD_MCP_BACKEND=api` 调 REST；可设 `db` 直连。列表/搜索工具支持 `limit` / `offset` 分页。`AGENTBOARD_MCP_TRANSPORT=http` 提供 `/mcp` Streamable HTTP 端点；远程端点默认验证与 REST 登录相同的 Bearer Token，并把当前请求 Token 透传给受保护 REST API。
