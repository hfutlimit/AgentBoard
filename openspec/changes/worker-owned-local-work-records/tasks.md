...
- [ ] claim 接受后、Provider 前 durable CreateRunning；写入失败时 adapter 零调用，按既有 fail/retry 边界安全退出。
- [ ] 接入 journal-save 后 pending、completion 后 success、已知失败后 failed；每个变更追加 event 并保证重试幂等。
- [ ] 实现持锁启动恢复和 journal/history 对账：running 转 interrupted，journal 有结果只补 pending/发布/completion，不重调 Provider；处理 pending 落库和 completion-success 落库失败窗口。
- [ ] 保持当前 journal cleanup：成功 completion 不 Remove，只有既有明确 fail 成功和 new-token-required 冲突 Remove；不改旧存储/API、队列 ACK 和 drain。
...
- [ ] 增加 Worker 故障窗口测试：pre-provider 写入失败、journal/pending 失败、completion 成功后 success 写入失败、timeout replay、interrupted 恢复及 journal cleanup 兼容。
