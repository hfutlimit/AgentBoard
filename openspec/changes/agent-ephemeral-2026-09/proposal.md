...
| **P2 server dispatch** | Dispatch paths read cache; 503 on miss. New optional Task.pinned_agent_id column (DB write allowed, **unlike** agent_instances). | +150 / -50 | existing e2e suite (1644-style + cross-owner) still green; new e2e: dispatch hits 503 then succeeds after worker re-registers |
