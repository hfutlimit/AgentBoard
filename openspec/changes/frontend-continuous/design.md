...
2. **范围红线**：单文件改动为主；新增前端代码 < ~80 行；不引入新 npm/打包依赖；不改 `models.py` / `api.py` 契约（除非该项明确标注「需后端」）。
3. **完成标准**：本地启动 `api` + `web` 服务，打开页面手测通过；若改动通用渲染（如 `md()`、`api()`、`statusSelect()`），跑一遍现有 `tests/test_web_flow.py` 与 Playwright 冒烟确保不回归。
4. **超限即拆**：若某项实际偏大，拆回更细子项，本轮只做其一，剩余回写 backlog（保持 unchecked）。
