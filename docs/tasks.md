...
- [x] **v1.8 可折叠任务分组**：B-06 分组后的组标题可点击折叠/展开。Angular `collapsedGroups` signal（`Set<string>`）+ `isGroupCollapsed()` / `toggleGroup()` 方法；HTML 组标题加 `task-group-chevron`（▸/▾）+ `(click)="toggleGroup(grp.key)"` + `[class.collapsed]`；`@if (!grp.key || !isGroupCollapsed(grp.key))` 包裹组内 `@for` 任务列表控制可见性；折叠状态持久化 `localStorage` 键 `agentboard_collapsed_groups`（JSON 数组）；CSS `.task-group-header` 加 `cursor:pointer` + hover 变色 + `.task-group-chevron` 旋转过渡。复用既有 `groupedTasks()` computed，零后端契约变更；Playwright E2E（`tests/test_v18_collapsible_groups_e2e.py`）折叠/展开/localStorage 持久化全绿、零 page/console/404 错误。净增 ~30 行（app.ts+10/app.html+4/app.css+4），符合 R2。Epic 29 / Story 65 / Task 837 全部 done。
